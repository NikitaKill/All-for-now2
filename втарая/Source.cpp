...
struct Priamoygolnik //2221 и 2222
{
	double dlina;
	double shirina;
	string cvet;
...
{
	Reis flt1, flt2, flt3, flt4;
	Reis flights[4] = { flt1, flt2, flt3, flt4 };
	int count = 4;
	delete[] flights;
}
...
struct Movie1 {
	...
	int time;
	int year;
	enum janre { Comedy, Drama, Thriller, Action, Horror, blockbuster };
	double rating;
};
//2292
...
void DemoMovieWithGenre() {
	...
	cin >> movie.year;
	cout << "Ведите жанр фильма (0 - Комедия, 1 - Драма, 2 - Триллер, 3 - Экшен, 4 - Хоррор, 5 - Блокбастер)" << endl;
	cin >> n;
	switch (n) {
	case 0:
		cout << "Комедия" << endl;
	...
	cin >> number;
	delete[] movie1;
	Movie1* movies = new Movie1[number];
	for (int i = 0; i < number; i++)
	{
		...
		cout << "Введите рейтинг фильма: " << endl;
		cin >> movies[i].rating;
		cout << "Введите жанр фильма (0 - Комедия, 1 - Триллер, 2 - Экшен, 3 - Хоррор)" << endl;
		int choice;
		cin >> choice;
		switch (choice) {
		case 0:
			movies[i].janre = Comedy;
			break;
		case 1:
			movies[i].janre = Thriller;
			break;
		case 2:
			movies[i].janre = Action;
			break;
		case 3:
			movies[i].janre = Horror;
			break;
		}
	}
	Genre findGenre = Comedy;
	int a;
	a = CountMoviesByGenre(movies, number, findGenre);
	cout << "Всего " << a << " фильмов жанра " << findGenre << endl;
	Movie1* Bmovie = new Movie1;
	Bmovie = FindBestGenreMovie(movies, number, findGenre);
}
Genre ReadGenre() {
	int n;
	cout << "Ведите жанр фильма (0 - Комедия, 1 - Драма, 2 - Триллер, 3 - Экшен, 4 - Хоррор, 5 - Блокбастер)" << endl;
	...
	for (int i = 0; i < count; i++)
	{
		if (movies[i].genre == findGenre)
		{
			if (movies[i].rating > rating1)
			{
				...
				cout << "Наивысший рейтинг: " << rating1;
			}
			else return nullptr;
		}
	}
